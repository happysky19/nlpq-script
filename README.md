...
- SW `rt-aware` requires official CKDMIP Rayleigh optical depth and solar
  ...
  diagnostic run.
- SW `rt-aware` uses py2sess `forward_flux` flux-only training with solar
  first-order correction by default. Keep `rt.sw_include_fo: true` and
  `rt.sw_plane_parallel: true` unless running a diagnostic ablation.
